import { ReplaySubject, Observable, race } from 'rxjs';
  ...
  return (target: any, propertyKey: string, descriptor) => {
    ...
    descriptor.value = function(...args) {
      ...
      /*
        ...
        ```
        if the function is called like`service.findAll();` then args would be [] but `originalFunction`
            will actually call the service with [1]
